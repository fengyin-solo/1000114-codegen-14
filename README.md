...
调度派单按岗位分权，身份目录在 `backend/app/auth.py`，请求头 `X-Principal` 传岗位身份键
（导出等无法带头的场景用 `?principal=`）；顶栏可直接切换岗位，切换后页面可见列与动作
入口立即按 `GET /api/dispatch/scope` 下发的边界刷新：

| 身份键 | 岗位 | 归属 | 边界 |
| --- | --- | --- | --- |
| `dispatcher_a` / `dispatcher_b` | 调度员 | 一/二车队 | 只能新建本车队调度单（车队锁定），仅本车队单子可派单/发车 |
| `manager_a` | 车队管理员 | 一车队 | 本车队单子可改（指派车辆/指派司机/计划发车时间）、可流转；其他车队只读 |
| `ops_admin` | 运营管理员 | 全部 | 可撤销任意一张单子；不参与新建/改单/派单/发车 |

字段范围：运营视角不含指派车辆/司机/计划发车时间；车队岗位对其他车队的单子隐去这些
执行列。所有写入由后端裁决，越权一律返回 `ok:false` 并在 `message` 注明原因。
